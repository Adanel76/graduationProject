...
const normalizeGalleryImages = (tour) => {
  if (!tour) return [FALLBACK_IMAGE];

  const mainImage = buildImageSrc(tour);

  const rawGallery = Array.isArray(tour.gallery_images)
    ? tour.gallery_images
    : [];

  const galleryFromArray = rawGallery
    .map((item) => {
      if (typeof item === 'string') return item;

      if (item?.image_data) {
        return `data:${item.image_type || 'image/jpeg'};base64,${item.image_data}`;
      }
...
const buildMapUrl = (tour, extraText = '') => {
  const lat = getTourLatitude(tour);
  const lng = getTourLongitude(tour);

  if (lat && lng) {
    return `https://yandex.ru/map-widget/v1/?ll=${lng}%2C${lat}&z=12&pt=${lng}%2C${lat}%2Cpm2rdm`;
  }

  const query = [tour?.city, tour?.country, extraText]
    .filter(Boolean)
    .join(', ');
...
const buildYandexMapOpenUrl = (tour, extraText = '') => {
  const lat = getTourLatitude(tour);
  const lng = getTourLongitude(tour);

  if (lat && lng) {
    return `https://yandex.ru/maps/?ll=${lng}%2C${lat}&z=12&pt=${lng}%2C${lat}%2Cpm2rdm`;
  }
...
const TourDetail = () => {
  ...
  useEffect(() => {
    if (!activeInfoBlock) return undefined;
  ...
  const galleryImages = useMemo(() => {
    return normalizeGalleryImages(tour);
  }, [tour]);
    ...
    return [
      {
        ...
        note: 'Детали размещения могут зависеть от выбранного пакета и наличия мест.',
      },
      {
        key: 'meals',
        icon: '🍽️',
        ...
        note: 'Точный формат питания подтверждается менеджером при обработке заявки.',
      },
      {
        key: 'activities',
        icon: '🎯',
        ...
        note: 'Программа может корректироваться в зависимости от сезона и погодных условий.',
      },
      {
        key: 'resort_info',
        icon: '🗺️',
        ...
        mapHint: 'центр',
        note: 'Рекомендуем заранее уточнять особенности направления перед поездкой.',
      },
    ];
  }, [tour]);

  const selectedBlock = includedBlocks.find(
    (block) => block.key === activeInfoBlock
  );
            ...
            <div className="tour-detail-gallery__main">
              ...
              {galleryImages.length > 1 && (
                <>
                  <button
                    type="button"
                    className="tour-detail-gallery__arrow tour-detail-gallery__arrow--prev"
                  ...
                  <button
                    type="button"
                    className="tour-detail-gallery__arrow tour-detail-gallery__arrow--next"
                  ...
                  <div className="tour-detail-gallery__dots">
              ...
              <div className="tour-detail-gallery__badge">
                ...
                <span>мест: {availablePlaces}</span>
              </div>
            </div>
          </div>
            ...
            <section className="tour-detail-card">
              <div className="tour-detail-section-head">
                <div>
                  ...
                  <p>
                    {getTourLatitude(tour) && getTourLongitude(tour)
                      ? 'Точка направления задана менеджером или администратором.'
                      : 'Карта построена по названию города и страны.'}
                  </p>
                </div>

                <a
                  className="site-button site-button--secondary"
                  href={buildYandexMapOpenUrl(tour)}
                  target="_blank"
                  rel="noreferrer"
                >
          ...
          <aside className="tour-detail-sidebar">
            <section className="tour-detail-card tour-detail-sticky">
              ...
              <button
                ...
                disabled={availablePlaces <= 0}
              >
                {availablePlaces > 0 ? 'Забронировать' : 'Мест нет'}
              </button>
              ...
              {bookingVisible && (
                <div className="tour-booking-box">
                  ...
                  <div className="tour-booking-box__total">
                    ...
                    <strong>
                      {formatMoney(
                        Number(tour.price || 0) * Number(peopleCount || 0)
                      )}
                    </strong>
                  </div>
      ...
      {selectedBlock && (
        ...
        >
          <div className="tour-info-modal__panel">
            <div className="tour-info-modal__header">
            ...
            <div className="tour-info-modal__body">
              <div className="tour-info-modal__intro">
                <span className="tour-info-modal__icon">
                  {selectedBlock.icon}
                </span>
              ...
              <div className="tour-info-modal__notice">
                <strong>Важно:</strong>
                <span>{selectedBlock.note}</span>
              </div>

              <div className="tour-info-modal__map-head">
                <div>
                  <h3>Локация на карте</h3>
                  <p>
                    {getTourLatitude(tour) && getTourLongitude(tour)
                      ? 'На карте показана точка, заданная сотрудником агентства.'
                      : 'Карта построена автоматически по направлению тура.'}
                  </p>
                </div>

                <a
                  href={buildYandexMapOpenUrl(tour, selectedBlock.mapHint)}
                  target="_blank"
                  rel="noreferrer"
                  className="site-button site-button--secondary"
                >
              ...
              <div className="tour-map tour-map--modal">
                <iframe
